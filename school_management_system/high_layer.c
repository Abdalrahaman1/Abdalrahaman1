...
void    New_Student         (void)
{
    ...
    {                                                                               /*list not full*/
        ...
        /*check for id authenticity*/
        do
        {
            printf("Student ID: ");
            scanf("%hu", &temp_student.id);
            for(Ptn = l.alpha_order; ((l.list_size) && ((Ptn ->student.id != temp_student.id) && (Ptn != l.top))); Ptn = Ptn -> next){}
            if((l.list_size) && (Ptn ->student.id == temp_student.id))
            {
